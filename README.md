...
새로운 블럭 내에서 위의 x를 이용해서 let키워드로 x를 재정의 해 사용하는 것을 볼 수 있다. 그렇게 선언된 x는 종속된 코드블럭 내에서 유효하며, 범위를 벗어나면 다시 원래의 x값을 가지게 된다. 사실상 컨텍스트 스위칭에 의한 새로운 변수로 볼 수 있겠다.
...
러스트의 모든 값은 자료형을 가지게 된다. 이것은 러스트에게 어떤 종류의 데이터가 확정되어지는 지 알려주며, 이것은 데이터와 어떻게 작업 될 지 알 수 있다.

우리는 여기서 크게 두가지 자료형 부분집합 `Scalar`와 `Compound` 알아볼 것이다.

일단 러스트는 정적 타입언어임을 항상 머릿속에 기억하는 것이 좋다. 이것은 컴파일 타임에 모든 자료형을 알아야만 한다는 것을 뜻한다. 컴파일러는 실제 값과, 우리가 어떻게 사용하는지를 바탕으로 우리가 어떻게 사용하고싶은가를 대개 유추할 수 있다.
...
이 때, `isize`, `usize` 타입들은 은 당신의 프로그램이 돌아가는 컴퓨터의 구조에 따라 달라질 수 있습니다. 당신이 64비트 컴퓨터를 사용 할 경우 i/u64, 32비트 컴퓨터의 경우 i/u32 의 메모리를 할당받을 것입니다.

정수 데이터를 명시, 표기 할 때 여러 방식으로 표기할 수 있습니다.
이것을 정수 리터럴(Integer literal) 이라고 하며 다음과 같이 표기할 수 있습니다.  
(이 때, 여러 숫자 유형이 될 수 있다면, `58u8` 과 같이 유형 접미사를 허용합니다.)  
뿐만아니라 숫자를 읽기 쉽도록 `_` (언더바) 로 구분할 수 있습니다.

![정수 리터럴](./readme/integer-literal.png)

정수 유형의 기본 값은 i32 이며,`isize`, `usize` 등을 사용하는 주요 상황은 어떤 컬렉션을 인덱싱 할 때 입니다.

### 부동 소수점 자료형

러스트는 물론 두개의 부동 소수점 자료형을 지원합니다. 십진수 소수점을 가집니다.  
`f32`, `f64` 두가지 자료형을 지원하며, 기본은 `f64` 를 사용합니다.
최근 CPU 는 처리속도는 비슷하지만 정확도가 더 높기 때문입니다.
...
다른 프로그래밍 랭귀지 처럼 러스트도 불리언 타입을 지원합니다. 1바이트의 사이즈를 가지며, 타입을 명시할 시 `bool` 키워드를 사용합니다.
...
작은 따옴표로 리터럴 지정합니다. 크리가 4바이트입니다. (기존 C와 같은 언어는 1바이트)
유니코드 스칼라 값을 가지기 때문에 이모티콘 같은 것들, 한국어, 일본어 등을 지원합니다.
...
튜플은 단일 복합요소로 간주되어, tup 라는 변수에 바인딩 됩니다.
따라서 별개의 밧을 얻고 싶은 경우 다음과 같은 매칭 패턴을 사용할 수 있습니다.
...
러스트는 다른언어와 달리 불리언이 아닌 유형을 자동으로 변환하지 않음. 명시적어야하며 if 문과 같은 조건문들은 항상 bool 값을 condition으로 제공해야 함.
